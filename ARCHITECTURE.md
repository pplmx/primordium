...
每个代理拥有一个 **6-6-5** 结构的神经网络（进化自早期的 4-6-4）：

```rust
pub struct Brain {
    pub weights_ih: [f32; 36],  // 6 inputs -> 6 hidden
    pub weights_ho: [f32; 30],  // 6 hidden -> 5 outputs
    pub bias_h: [f32; 6],
    pub bias_o: [f32; 5],
}
```
...
### 4. 昼夜循环 (Circadian Rhythms) [NEW]
- **光照影响**: 植物生长概率与当前光照强度 (`light_level`) 成正比。正午达到峰值，深夜几乎停止。
- **代谢波动**: 实体在夜间进入低代谢模式，静息能量消耗降低 40%。
- **周期设定**: 默认为 2000 ticks 一个完整周期。
从 Phase 14 开始，项目进行了深度的模块化重构：

### 1. `src/app/`：应用层
- `mod.rs`：主运行循环逻辑。
- `state.rs`：应用状态定义及初始化。
- `render.rs`：基于 `ratatui` 的 TUI 渲染代码。
- `input.rs`：键盘与鼠标交互处理。
- `onboarding.rs` / `help.rs`：新手指引与帮助系统。

### 2. `src/model/`：领域驱动逻辑
- `world.rs`：物理模拟核心。
- `brain.rs`：神经网络与遗传算法实现。
- `entity.rs`：生命体定义。
- `environment.rs`：硬件指标到气候的映射。
- `quadtree.rs`：空间哈希优化索引。
...
1. **静态分析**：强制执行 `cargo clippy -- -D warnings`，保持 0 警告。
2. **代码风格**：统一使用 `cargo fmt`。
3. **分层测试**：
   - **单元测试**：核心算法（Brain, Hash）的边界验证。
   - **集成测试**：完整的生命周期流程（`tests/simulation_logic.rs`）、DNA 解析协议（`tests/genetic_flow.rs`）。
4. **Git Hooks**：通过 Husky 在 `pre-commit` 阶段运行全量检查，确保破坏性更改无法进入仓库。
