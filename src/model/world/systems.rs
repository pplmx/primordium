...
struct EntityPerceptionInput<'a> {
    identity: &'a primordium_data::Identity,
    pos: &'a primordium_data::Position,
    phys: &'a primordium_data::Physics,
    met: &'a primordium_data::Metabolism,
    intel: &'a mut primordium_data::Intel,
    health: &'a primordium_data::Health,
}

struct EntityCommandInput<'a> {
    i: usize,
    identity: &'a primordium_data::Identity,
    pos: &'a primordium_data::Position,
    phys: &'a primordium_data::Physics,
    met: &'a primordium_data::Metabolism,
    intel: &'a primordium_data::Intel,
    health: &'a primordium_data::Health,
    decision: &'a EntityDecision,
}
    ...
    entity_data
        ...
        .for_each(
            |(_i, ((_handle, (identity, pos, _vel, phys, met, intel, health)), decision))| {
                let input = EntityPerceptionInput {
                    identity,
                    pos,
                    phys,
                    met,
                ...
                *decision = perceive_one_entity(input, ctx, env, id_map);
            },
        );
    ...
    let all_cmds_flat: Vec<InteractionCommand> = entity_data
        ...
        .fold(
            ...
            |mut acc, (i, (_handle, (identity, pos, _vel, phys, met, intel, health)))| {
                ...
                let cmds =
                    generate_commands_for_entity(input, ctx, env, biomass_c, id_map, pop_len);
                acc.extend(cmds);
                acc
            },
        )
...
fn generate_commands_for_entity(
    ...
    if outputs[4] > ctx.config.social.aggression_threshold && met.energy > met.max_energy * 0.7 {
            ...
            if i != t_idx {
                ...
                if color_dist >= ctx.config.social.tribe_color_threshold {
                    ...
                    let competition_mult = (1.0
                        - (biomass_c / ctx.config.ecosystem.predation_competition_scale))
    ...
    if lifecycle::is_mature_components(met, intel, ctx.tick, ctx.config.metabolism.maturity_age)
        && met.energy > ctx.config.metabolism.reproduction_threshold
    {
        ...
        let mut modified_genotype = intel.genotype.clone();
        modified_genotype.reproductive_investment =
            (modified_genotype.reproductive_investment * decision.grn_repro_mod).clamp(0.1, 0.9);
