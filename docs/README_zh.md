...
每个个体都拥有一个 **6-6-5 结构神经网络** 大脑。通过自然选择，生物会学会狩猎、导航和能量管理。

- **交互式选择**：直接使用 **鼠标** 点击屏幕上的生物进行锁定和深度检视。
- **基因型聚类**：系统根据神经权重相似度自动将生物归类为不同的"物种"。
- **图例栏 (Legend Bar)**：TUI 顶部新增第四行，实时显示状态与地形符号含义。
...
### ⚡ 性能与稳定性
- **模块化架构**: 状态管理、渲染逻辑与物理模拟核心解耦，代码更清晰。
- **严苛质量门禁**: 100% Clippy 通过率，全方位单元与集成测试保障。
